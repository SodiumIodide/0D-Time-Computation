...
function main()::Nothing
    ...
    local intensity_2_bin::Vector{Histogram.Hist} = [Histogram.Hist(num_bins, intensity_2_min, intensity_2_max) for i in 1:nthreads()]
    local temperature_1_bin::Vector{Histogram.Hist} = [Histogram.Hist(num_bins, temperature_1_min, temperature_1_max) for i in 1:nthreads()]
    local temperature_2_bin::Vector{Histogram.Hist} = [Histogram.Hist(num_bins, temperature_2_min, temperature_2_max) for i in 1:nthreads()]
    local opacity_1_bin::Vector{Histogram.Hist} = [Histogram.Hist(num_bins, opacity_1_min, opacity_1_max) for i in 1:nthreads()]
    local opacity_2_bin::Vector{Histogram.Hist} = [Histogram.Hist(num_bins, opacity_2_min, opacity_2_max) for i in 1:nthreads()]
