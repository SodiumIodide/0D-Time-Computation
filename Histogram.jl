module Histogram
    ...
    mutable struct Hist
        ...
        Hist(len::Int64, min::Float64, max::Float64) = new(len, min, max, zeros(len + 1), (max - min) / convert(Float64, len))
    end
    ...
    function distribution(his::Hist)::Vector{Float64}
        return vec([i * his.m_delta + his.m_min - his.m_delta / 2.0 for i in 1:(his.m_length + 1)])
    end
end
