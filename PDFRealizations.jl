...
function main()::Nothing
    ...
    local data_location::String = "out/nonlinear/data/nonlinear.csv"
    ...
    if (isfile(data_location))
        old_data = CSV.File(data_location) |> DataFrame
    else
        println("Cannot find results of a Realizations run, terminating")
        return nothing
    end
